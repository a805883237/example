...
import {getShop, getshops, saveShop, updateShop} from '../../reducers/actions/shops';
...
function formMapStateToProps(state) {
  return {
    itemData: state.shops.itemData
  };
}
//redux动作
function mapDispatchToProps(dispatch) {
  return {
    getItemData: (params) => dispatch(getShop(params)),
    saveItemData: (params)=>{
       dispatch(saveShop(params)).then(()=> {
         dispatch(getShops())
       })
    },
    updateItemData: (id, params) => dispatch(updateShop(id, params))
  }
}
MyForm = createForm({
  mapPropsToFields(props) {
    ...
    for (let prop in props.itemData) {
       result["shop." + prop] = {value: props.itemData[prop]}
    }
    return result;
        }
  }
