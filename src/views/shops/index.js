...
import {getShops,delete} from '../../reducers/actions/shops';
import {Input, Button, Popconfirm, Row, Col} from 'antd';
import ShopNew from './new.js';
...
export class ShopsIndex extends React.Component {
  constructor(props) {
    super(props);
        ...
        title: '操作', key: 'operation', render: (item) => (
          <div className="actions">
            ...
            <Popconfirm title="确定删除？" placement="right"
                        onConfirm={this.props.deleteShops.bind(this, {id: item.id})}>
    ...
    return (
        <div>
          ...
          <Row>
            <Col span={10}>
              <div style={{marginBottom: 16}} className="actions-control">
                ...
                <Popconfirm title="确定删除？" placement="right" onConfirm={this.props.deleteShops.bind(this)}>
          ...
          <CustomTable
              ...
              bordered
          />
          <ShopNew ref="newForm" title="新建" formType="newForm"></ShopNew>
          <ShopNew ref="editForm" title="编辑" formType="editForm"></ShopNew>
        </div>
    );
  }
}
...
function mapStateToProps(state) {
  return {data: state.shops.data};
}

function mapDispatchToProps(dispatch) {
  return {
    getShops: (params) => dispatch(getShops(params)),
    deleteShops:  (params) =>
    {
      dispatch(deleteShops(params)).then(() => {
        dispatch(getShops());
      })
    },
