...
var ImageLibDialog = require('../plugins/ImageLibDialog.react');
...
var EditorCore = function (_React$Component) {
	...
	_createClass(EditorCore, [{
      ...
      console.log("hhhhhhhh",state.icon)
      var editorState = this.state.editorState;
      ...
      console.log("sssssss",state.icon)
			switch (state.icon) {
				case "source":
        ...
        case "imageregion":
					EditorSelection.storeRange();
          // this.refs.imageregion.setState({visible: true})
					this.refs.imageregion.toggle(function (e, html) {
						editarea.focus();
						EditorSelection.restoreRange();
						if (html && html.length > 0) {
							if (EditorSelection.range) {
		...
		value: function closeAllOpenDialog(icon) {
			var refsDialog = [ "color", "formula", "table", "special", "emotion", "fontsize", "fontfamily", "paragraph"];
		...
		value: function genEditArea() {
			...
			if (showHtml) {
				return React.createElement(EditorTextArea, { ref: 'editarea', onChange: this.props.onChange.bind(this) });
			} else {
		...
		value: function render() {
			...
			var _props = this.props,
			    ...
			    props = _objectWithoutProperties(_props, ['index', 'fontSize', 'paragraph', 'fontFamily', 'icons', 'plugins', 'onBlur', 'className', 'id', 'onFocus', 'onClick', 'onEditorMount']);
			...
			return React.createElement(
				...
				React.createElement(
					...
					React.createElement(ImageLibDialog, { hidden: _icons.indexOf("imageregion") == -1, ref: 'imageregion' }),
